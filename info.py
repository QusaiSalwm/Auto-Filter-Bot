...
# Bot information
API_ID = environ.get('API_ID', '26677274')  #api id of your telegram id
if len(API_ID) == 0:
    print('Error - API_ID is missing, exiting now')
    exit()
else:
    API_ID = int(API_ID)
API_HASH = environ.get('API_HASH', '5118f277677e273b90a7bd866d5b7731') #api hash of your telegram id
if len(API_HASH) == 0:
    print('Error - API_HASH is missing, exiting now')
    exit()
BOT_TOKEN = environ.get('BOT_TOKEN', '7068152758:AAHz_zNMQLkeKtJezuvgLZrZ-4KBKWssmus') #bot token from botfather
if len(BOT_TOKEN) == 0:
...
# Bot Admins
ADMINS = environ.get('ADMINS', '819385459') #apni tg id daalo
if len(ADMINS) == 0:
...
# Channels
INDEX_CHANNELS = [int(index_channels) if index_channels.startswith("-") else index_channels for index_channels in environ.get('INDEX_CHANNELS', '-1002236719145').split()]
...
LOG_CHANNEL = environ.get('LOG_CHANNEL', '-1002476811821') #bot log channel -1005293546253
...
# support group
SUPPORT_GROUP = environ.get('SUPPORT_GROUP', '-1002014591480') #support group id ex:  -1002936246860
...
# MongoDB information
DATABASE_URL = environ.get('DATABASE_URL', "mongodb+srv://autoFilterBotV2:autoFilterBot12345@cluster0.micrza8.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0") #mongo db url
if len(DATABASE_URL) == 0:
    print('Error - DATABASE_URL is missing, exiting now')
    exit()
DATABASE_NAME = environ.get('DATABASE_NAME', "autoFilterBotDB")
COLLECTION_NAME = environ.get('COLLECTION_NAME', 'autoFilterBotFiles')
...
BIN_CHANNEL = environ.get("BIN_CHANNEL", "-1002474358239") #if is_stream = true then add a channel id ex: -10026393639
if len(BIN_CHANNEL) == 0:
    print('Error - BIN_CHANNEL is missing, exiting now')
    exit()
else:
    BIN_CHANNEL = int(BIN_CHANNEL)
URL = environ.get("URL", "https://t.me/knbctjknvdthj") #if heroku then paste the app link here ex: https://heroku......./
if len(URL) == 0:
